...
class GeneralSeq2Seq(nn.Module):
    def __init__(
        ...
        self.encoder1 =  Encoder(
            input_dim=input_size, hidden_dim=hidden_size, input_channels=cnn_size, mode=mode
        ).to(device)
        self.decoder1 = Decoder(output_dim=output_size, hidden_dim=hidden_size).to(device)
        self.encoder2 = Encoder(input_dim=1, hidden_dim=hidden_size, mode=mode).to(device) if mode!="single" else None
        self.decoder2 = Decoder(output_dim=1, hidden_dim=hidden_size).to(device) if mode!="single" else None
        self.device = device
        self.mode = mode
    ...
    def process_single(self, src, trg_len, trg_start_token):
        ...
        trg_vocab_size = self.decoder1.output_dim
    ...
    def process_dual(self, src, trg_len, trg_start_token):
        src1, src2 = src
        # trg_start_token1, trg_start_token2 = trg_start_token
        encoder_outputs1, hidden1, cell1 = self.encoder1(src1)
        outputs1 = torch.zeros(trg_len, src1.shape[0], self.decoder1.output_dim).to(
            self.device
        )
        current_input = trg_start_token
        ...
        outputs2 = torch.zeros(trg_len, src2.shape[0], self.decoder2.output_dim).to(
            self.device
        )
        current_input = trg_start_token
