...
class MTSLSTM(nn.Module):
    ...
    def __init__(
        ...
        self.linear2 = nn.ModuleList([
            nn.Linear(41, 128) for i in range(self.nf)
        ])
            ...
            if not os.path.isfile(self.pretrained_day_path):
                ...
                try:
                    ...
                    if "linearIn.weight" in state and "linearIn.bias" in state:
                        ...
                        if target_linear.weight.shape == linear_weight.shape and target_linear.bias.shape == linear_bias.shape:
                            target_linear.weight.data.copy_(linear_weight)
                            target_linear.bias.data.copy_(linear_bias)
                            print(f"[MTSLSTM] Pretrained linearIn loaded into linear2[1]")
                        else:
                            warnings.warn(f"[MTSLSTM] linearIn shape mismatch: pretrained {linear_weight.shape}, "
